...
fn test_delete_client() {
    ...
    let clients = database::get_clients(&connection).unwrap();
    let client_nickname = clients[0].nickname.to_string();
...
fn test_new_and_get_invoice() {
    ...
    let client_id = database::get_clients(&connection).unwrap()[0]
        .id
        .to_string();
    let client_nickname = database::get_clients(&connection).unwrap()[0]
        .nickname
        .to_string();
    let date = "2025-06-06";
